...
FUN_RESULT Ping(const char * IpAddress)
{
	...
	if (iReadDataLen != SOCKET_ERROR)//有数据达到
	{
		...
		if (DecodeIcmpResponse(IcmpRecvBuf, iReadDataLen, DecodeResult, ICMP_ECHO_REPLY, ICMP_TIMEOUT))
		{
			...
			if (DecodeResult.dwIPaddr.s_addr == destSockAddr.sin_addr.s_addr) {
				//输出IP地址
				FUN_RESULT res = { DecodeResult.dwRoundTripTime, DecodeResult.dwIPaddr, PSUCC, iTTL, 32};
				return res;
	...
	{
		FUN_RESULT res;
		res.state = PERR_TIMEOUT;
		return res;
	}
	else {
		FUN_RESULT res;
		res.state = PERR_OTHER;
		return res;
	}
	WSACleanup();
}